...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const NUMERIC_PARAMS = [
      { key: "temperature", zh: "温度", en: "Temperature", min: 0, max: 2, step: 0.01, slider: true },
      { key: "top_p", zh: "Top-P", en: "Top-P", min: 0, max: 1, step: 0.01, slider: true },
      { key: "maxTokens", zh: "最大 Token 数", en: "Max tokens", min: 0, max: 1000000, step: 1, slider: false, zeroIsUnlimited: true },
    ];
    ...
    function SliderField(props) {
      ...
      return React.createElement("span", { style: { display: "inline-flex", alignItems: "center", gap: "8px" } },
        ...
        React.createElement("input", {
          ...
          disabled, style: { width: width || "84px" },
          onChange: (e) => { const v = clamp(e.target.value, min, max); if (Number.isFinite(v)) onChange(v); },
        }),
      );
    }

    /** Token-count control: a toggle (limit on/off) + an integer input when limited. `<= 0` = unlimited. */
    function TokenField(props) {
      const { value, disabled, onChange, max } = props;
      const unlimited = value === undefined || value <= 0;
      const toggle = () => {
        if (unlimited) onChange(8192);
        else onChange(0);
      };
      return React.createElement("span", { style: { display: "inline-flex", alignItems: "center", gap: "8px" } },
        React.createElement("button", {
          type: "button", disabled, onClick: toggle,
          style: { cursor: "pointer", padding: "2px 10px", borderRadius: "6px", border: "1px solid rgba(128,128,128,.35)", background: "transparent", color: "inherit", font: "inherit", fontSize: "13px", whiteSpace: "nowrap" },
        }, unlimited ? "不限 / Unlimited" : "限制 / Limited"),
        unlimited
          ? null
          : React.createElement("input", {
            className: "dsh-param-num", type: "number", min: 1, max, step: 1,
            value: String(value), disabled, style: { width: "90px" },
            onChange: (e) => { const v = Number(e.target.value); if (Number.isFinite(v)) onChange(Math.max(0, v)); },
          }),
      );
    }
          ...
          React.createElement("span", { style: fieldStyle },
            ...
            p.zeroIsUnlimited
              ? React.createElement(TokenField, {
                value: value.defaults[p.key], max: p.max,
                disabled: !writable || busy, onChange: onDefault(p.key, p.min, p.max),
              })
              : React.createElement(SliderField, {
                min: p.min, max: p.max, step: p.step, useSlider: p.slider !== false,
    ...
    function ParameterDock(props) {
      ...
      const chips = visible.map((p) => React.createElement("label", { key: p.key, className: "dsh-param-chip" },
        ...
        p.zeroIsUnlimited
          ? React.createElement(TokenField, {
            value: effective(p.key), max: p.max,
            disabled: !writable, onChange: onEdit(p.key, p.min, p.max),
          })
          : React.createElement(SliderField, {
            min: p.min, max: p.max, step: p.step, useSlider: p.slider !== false,
